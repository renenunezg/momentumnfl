...
def main() -> None:
    ...
    if args.command == "ingest":
        ...
        run_bootstrap_history(args)
    elif args.command == "refresh-inputs":
        run_refresh_inputs(args)
...
def run_refresh_inputs(args) -> None:
    ...
    problems = ingest_projection_inputs(args.season)
    for problem in problems:
        print(f"PROBLEM: {problem}")
    if problems:
        raise SystemExit(1)
...
def run_preseason(args) -> None:
    ...
    projections_only = getattr(args, "projections_only", False)
    ...
    for season in sorted(args.seasons):
        try:
            ...
            if pfr_pass is not None and args.incremental:
                pfr_pass = pfr_pass[
                    pfr_pass["game_id"].astype(str).isin(game_ids)
                ]
            ...
            if args.incremental:
                ...
                store.write_processed(
                    model_games, "team_games", f"{season}.parquet"
                )
                store.write_processed(
                    qb_games, "qb_games", f"{season}.parquet"
                )
                store.write_processed(
                    unit_games, "unit_games", f"{season}.parquet"
                )
