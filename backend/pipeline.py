...
from backend.features.drives import _kickoff_utc, build_team_games
...
def bootstrap_history(through_season: int) -> list[int]:
    ...
    for season in missing:
        ...
        print(
            f"bootstrapped {season}: {len(model_games)} games; raw PBP discarded"
        )
    return missing
...
def upcoming_games(
    ...
    return slate[
        slate["start_date"].ge(now) & slate["start_date"].le(end)
...
def incremental_game_ids(
    ...
    recent = set(
        played.loc[played["week"].isin(recent_weeks), "game_id"]
    )
    complete = (
        set.intersection(*existing_game_ids) if existing_game_ids else set()
    )
...
def write_incremental_features(
    ...
    try:
        ...
        retained = existing[
            ~existing["game_id"].astype(str).isin(game_ids)
        ]
