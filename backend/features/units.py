...
from backend.features.drives import SCRIMMAGE_PLAY_TYPES, competitive_plays
...
def build_unit_games(
    ...
    scrimmage = scrimmage[competitive_plays(scrimmage)]

    rush = scrimmage[scrimmage["rush_attempt"].eq(1)]
    dropback = scrimmage[scrimmage["qb_dropback"].eq(1)]
    protection = dropback[dropback["sack"].eq(1) | dropback["qb_hit"].eq(1)]
    special = pbp[pbp["special"].eq(1) & pbp["epa"].notna() & pbp["posteam"].notna()]

    def per_team(frame: pd.DataFrame, name: str) -> pd.Series:
        return frame.groupby(["game_id", "posteam"])["epa"].sum().rename(name)
    ...
    if ngs_passing is not None and not ngs_passing.empty:
        ...
        ttt = (
            weekly.groupby(["season", "week", "team_abbr"])["avg_time_to_throw"]
            .mean()
            .reset_index()
